...
# ---------------------------------------------------
# Excel + Dictionary version
# ---------------------------------------------------

def generate_diff_excel_from_dict(old_excel_file, new_data_dict, output_file):
    """
    old_excel_file : path to existing Excel
    new_data_dict  : {sheet_name: pandas.DataFrame}
    output_file    : output excel path
    """

    print("Reading old workbook...")
    old_sheets = pd.read_excel(old_excel_file, sheet_name=None, engine="openpyxl")

    # ensure all values are dataframe
    for k, v in new_data_dict.items():
        if not isinstance(v, pd.DataFrame):
            raise ValueError(f"Value for sheet '{k}' must be a pandas DataFrame")

    # union of sheet names
    all_sheet_names = list(OrderedDict.fromkeys(list(old_sheets.keys()) + list(new_data_dict.keys())))
    ...
    with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
        ...
        for sheet in all_sheet_names:
            ...
            old_df = old_sheets.get(sheet, pd.DataFrame())
            new_df = new_data_dict.get(sheet, pd.DataFrame())
            ...
            if old_df.empty:
                ...
                final_df.to_excel(writer, sheet_name=sheet, index=False)
                ws = writer.sheets[sheet]
                ws.set_column(0, len(final_df.columns)-1, None,
    ...
    print("\nFinished (dict mode) →", output_file)
...
if __name__ == "__main__":
    ...
    # Example simulated dictionary input
    new_data_dict = {
        "Sheet1": pd.read_excel("New_Version.xlsx", sheet_name="Sheet1")
    }

    generate_diff_excel_from_dict(
        old_excel_file=OLD_FILE,
        new_data_dict=new_data_dict,
        output_file="Final_From_Dict.xlsx"
    )
